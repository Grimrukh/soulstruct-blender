...
class GameStructure:
    ...
    def get_map_file_path(self, *parts, if_exist=False, dcx_type: DCXType = None, map_stem: str = None) -> Path | None:
        ...
        map_stem = self._process_map_stem(map_stem)
        if not map_stem:
            return None
    ...
    def get_map_dir_path(self, *parts, if_exist=False, map_stem: str = None) -> Path | None:
        """Get the `map/{map_stem}` path, and optionally further, in the game directory.
    ...
    def get_msb_path(self, if_exist=False, map_stem: str = None) -> Path | None:
        ...
        if not map_stem:
            return None
        relative_msb_path = self.settings.get_relative_msb_path(map_stem)
    ...
    def _process_map_stem(self, map_stem: str = None) -> str | None:
        if map_stem is None:
            # Use selected map stem.
            map_stem = self.map_stem
        if not map_stem:
            # No map stem selected.
            return None
        return self.settings.process_file_map_stem_version(map_stem)
