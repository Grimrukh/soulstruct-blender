...
def get_armature_frames(animation_hkx: AnimationHKX, skeleton_hkx: SkeletonHKX, track_bone_names: list[str]):
    """Get a list of animation frame dictionaries, which each map bone names to their world transforms that frame."""
    anibnd = ANIBND(skeleton_hkx=skeleton_hkx, animations_hkx={0: animation_hkx})
    world_frames = []
    for frame_index in range(len(anibnd.get_animation(0).animation_container.interleaved_data)):
