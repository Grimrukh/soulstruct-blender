...
from soulstruct.darksouls1ptde.maps.msb import MSBEvent
...
class BlenderMSBEvent(SoulstructObject[MSBEvent, MSBEventProps], tp.Generic[EVENT_T, SUBTYPE_PROPS_T]):
    ...
    def unknowns(self) -> list[int]:
        return list(self.type_properties.unknowns)

    @unknowns.setter
    def unknowns(self, value: list[int]):
        if len(value) != 4:
            raise ValueError("Unknowns must be a list of four integers.")
        self.type_properties.unknowns = value
    ...
    def new_from_soulstruct_obj(
        ...
        bl_event.unknowns = soulstruct_obj.unknowns
