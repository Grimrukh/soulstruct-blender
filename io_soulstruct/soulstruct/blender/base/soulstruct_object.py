...
class BaseBlenderSoulstructObject(abc.ABC, tp.Generic[SOULSTRUCT_T, TYPE_PROPS_T]):
    ...
    def new_from_soulstruct_obj(
        ...
        if not isinstance(soulstruct_obj, cls.SOULSTRUCT_CLASS):
            cls_name = soulstruct_obj.__class__.__name__
            raise TypeError(
    ...
    @property
    def name(self):
        """Straightforward wrapper for Blender object name."""
...
def add_auto_type_props(cls: type, *names: str):
    """Create getter/setter properties for all `names` within `self.type_properties` in `cls`.

    `cls` type must have a `type_properties` `PropertyGroup` attribute, such as `BaseBlenderSoulstructObject` subtypes.
    """
    if not hasattr(cls, "type_properties"):
        raise AttributeError(
            f"Class {cls.__name__} must have a 'type_properties' attribute to use `add_auto_type_props`."
        )
