...
class LoggingImportOperator(LoggingOperator, ImportHelper):
    ...
    NOTE: Also used by some *export* operators to select existing Binder files to export *into*.
    """
...
class BinderEntrySelectOperator(LoggingOperator):
    ...
    def invoke(self, context, event):
        ...
        binder = self.get_binder(context)
        if binder is None:
            return self.error("No Binder could be loaded for entry import.")
        self.binder = binder

        self.temp_directory = tempfile.mkdtemp(suffix="_" + (self.binder.path_stem or "binder"))
